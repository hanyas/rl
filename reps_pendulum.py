...
class Environment:
	...
	def __init__(self, n_states, n_actions, action_limit, state_limit, goal, state_weight, action_weight):
	...
	def reset(self):
		self.x_intern = np.array([np.random.uniform(np.pi - np.pi/6.0, np.pi + np.pi/6.0), 0.0])
		self.x_intern = np.random.multivariate_normal(self.x_intern, 1e-8 * np.eye(2))

		xt = self.transform_angle(self.x_intern)
		xc = self.pol2cart(xt)
		return xc
	...
	def step(self, u):
		...
		r = self.reward(self.x_intern, uc)

		dt, g = 0.05, 9.81
		l, m, k = 0.5, 10.0, 0.25

		self.x_intern = sc.integrate.odeint(self.pendulum_ode, self.x_intern, np.array([0.0, dt]), args=(uc, g, m, l, k))[1, :]
...
class REPS:
	...
	def __init__(self, n_states, n_actions,
		...
		self.state_weight = np.diag([1e1, 1e1, 1e-1])
		self.action_weight = np.diag([1e-3])

		self.env = Environment(n_states, n_actions,
								action_limit=action_limit, state_limit=state_limit,
		...
		self.ctl.cov = (2.0 * action_limit)**2
		...
		while n < n_samples:
			...
			for t in range(n_steps):
				...
				if reset and init:
					...
					x_aux, r_aux = self.env.step(u_aux)
