...
def dual_eta(eta, omega, epsilon, gamma, nvfeatures, vfeatures, ivfeatures, r):
    adv = r + gamma * np.dot(nvfeatures, omega) - np.dot(vfeatures, omega) +\
          (1.0 - gamma) * np.mean(np.dot(ivfeatures, omega), axis=0, keepdims=True)
...
def dual_omega(omega, eta, epsilon, gamma, nvfeatures, vfeatures, ivfeatures,
               r):
    adv = r + gamma * np.dot(nvfeatures, omega) - np.dot(vfeatures, omega) + (
            1.0 - gamma) * np.mean(np.dot(ivfeatures, omega), axis=0,
                                   keepdims=True)
    delta = adv - np.max(adv)
    w = np.exp(np.clip(delta / eta, EXP_MIN, EXP_MAX))
...
def grad_omega(omega, eta, epsilon, gamma, nvfeatures, vfeatures, ivfeatures, r):
    ...
    delta = adv - np.max(adv)
    w = np.exp(np.clip(delta / eta, EXP_MIN, EXP_MAX))
    domega = (1.0 - gamma) * np.mean(ivfeatures, axis=0, keepdims=False) +\
             np.sum(w[:, None] * (gamma * nvfeatures - vfeatures), axis=0,
                    keepdims=False) / np.sum(w, axis=0, keepdims=False)
    return domega
...
class REPS:
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.ctl = Policy(self.n_states, self.n_actions, n_feat=self.n_pfeat,
                          band=np.array([1.25, 1.4, 0.7, 0.7, 1.85]))
    ...
    def kl_divergence(self):
        adv = self.data['r'] + self.discount * \
              ...
              np.dot(self.vfeatures, self.vfunc.omega) + (1.0 - self.discount) *\
              np.mean(np.dot(self.ivfeatures, self.vfunc.omega), axis=0, keepdims=True)
        delta = adv - np.max(adv)
...
for it in range(reps.n_iter):
    ...
    for _ in range(250):
        res = sc.optimize.minimize(dual_eta, reps.eta, method='L-BFGS-B',
                                   jac=grad_eta,
                                   args=(reps.vfunc.omega, reps.kl_bound,
                                         reps.discount, reps.nvfeatures,
                                         reps.vfeatures, reps.ivfeatures,
                                         reps.data['r']),
                                   bounds=((1e-8, 1e8),))
        # print(res)

        # check = sc.optimize.check_grad(dual_eta, grad_eta, res.x,
        #                                reps.vfunc.omega, reps.kl_bound,
        #                                reps.discount, reps.nvfeatures,
        #                                reps.vfeatures, reps.ivfeatures,
        #                                reps.data['r'])
        ...
        res = sc.optimize.minimize(dual_omega, reps.vfunc.omega, method='BFGS',
                                   jac=grad_omega,
                                   args=(
                                   reps.eta, reps.kl_bound, reps.discount,
                                   reps.nvfeatures, reps.vfeatures,
                                   reps.ivfeatures, reps.data['r']))

        # res = sc.optimize.minimize(dual_omega, reps.vfunc.omega,
        #                            method='trust-exact', jac=grad_omega,
        #                            hess=hess_omega,
        #                            args=(
        #                            reps.eta, reps.kl_bound, reps.discount,
        #                            reps.nvfeatures, reps.vfeatures,
        ...
        # print(res)
        #
        # check = sc.optimize.check_grad(dual_omega, grad_omega, res.x, reps.eta,
        #                                reps.kl_bound, reps.discount,
        #                                reps.nvfeatures, reps.vfeatures,
        #                                reps.ivfeatures, reps.data['r'])
    ...
    print('Iteration:', it, 'Reward:', eval['r'].sum() / reps.n_rollouts,
          'KL:', kl_div, 'Cov:', *reps.ctl.cov)
