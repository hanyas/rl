...
def learn_vfunction(vfeatures, omega, data, discount, trace):
	...
	for rev_k, v in enumerate(reversed(values)):
		...
		if data["done"][k]:
			...
			adv[k] = data["r"][k] + discount * values[k + 1] - values[k] + discount * trace * adv[k + 1]
...
class ACREPS:
	...
	def ml_policy(self):
		...
		from sklearn.linear_model import Ridge
		clf = Ridge(alpha=0.000001, fit_intercept=False, solver='sparse_cg', max_iter=2500, tol=1e-4)
...
acreps = ACREPS(n_states=5, n_actions=1,
				n_samples=10000, n_iter=15, n_keep=0,
				kl_bound=0.1, discount=0.99,
				n_vfeat=250, n_pfeat=250,
				n_rollouts=25, n_steps=1000)
...
for it in range(acreps.n_iter):
	...
	acreps.vfunc.omega = learn_vfunction(acreps.vfeatures, acreps.vfunc.omega, acreps.data, acreps.discount, 0.97)
	...
	print('Iteration:', it, 'Reward:', np.sum(eval['r']) / acreps.n_rollouts, 'KL:', kl_div, 'Cov:', *acreps.ctl.cov)
