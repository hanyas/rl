...
def learn_vfunction(vfeatures, omega, data, discount, trace):
    ...
    clf = SGDRegressor(alpha=0.0001, average=False, epsilon=0.1,
                       eta0=0.001, fit_intercept=False, l1_ratio=0.15,
                       learning_rate='invscaling', loss='squared_loss',
                       max_iter=None, n_iter=None, penalty='l2',
...
class ACREPS:
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.ctl = Policy(self.n_states, self.n_actions, n_feat=self.n_pfeat,
                          band=np.array([0.84, 1.32, 0.02, 0.2, 1.17]))
    ...
    def ml_policy(self):
        ...
        from sklearn.linear_model import SGDRegressor
        clf = SGDRegressor(alpha=0.0001, average=False, epsilon=0.1, eta0=0.001,
                           fit_intercept=False, l1_ratio=0.15,
                           learning_rate='invscaling', loss='squared_loss',
                           max_iter=None, n_iter=None, penalty='l2', power_t=0.25,
                           random_state=None, shuffle=True, tol=0.0001,
                           verbose=0, warm_start=False)
        clf.fit(psi, self.data['u'].flatten(), sample_weight=w)
        self.ctl.K = clf.coef_.reshape(1, -1)
...
acreps = ACREPS(n_states=5, n_actions=1,
                n_samples=5000, n_keep=0,
                n_iter=35, kl_bound=0.05, discount=0.985,
                n_vfeat=1000, n_pfeat=1000,
                n_rollouts=25, n_steps=500)
...
for it in range(acreps.n_iter):
    ...
    acreps.vfunc.omega = learn_vfunction(acreps.vfeatures, acreps.vfunc.omega,
                                         acreps.data, acreps.discount, 0.97)
...
acreps.env = None
acreps.env = gym.make("InvertedPendulumSwingupBulletEnv-v0")
acreps.env.render(mode="human")
