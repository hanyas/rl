...
def sample(env, nb_rollouts, nb_steps, dm_obs, dm_act):
    ...
    for n in range(nb_rollouts):
        ...
        for t in range(nb_steps):
            u = np.random.uniform(-10., 10., size=(1,))
...
if __name__ == "__main__":
    ...
    ararhmm = arARHMM(nb_states=nb_states,
                    dm_obs=dm_obs,
                    dm_act=dm_act,
                    type='recurrent')

    ararhmm.initialize(obs, act)
    lls = ararhmm.em(obs=obs, act=act, nb_iter=50, prec=1e-4, verbose=True)

    # path = os.path.dirname(rl.__file__)
    # pickle.dump(rarhmm, open(path + '/envs/control/hybrid/models/hybrid_msd.p', 'wb'))
