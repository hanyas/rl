...
class BaumWelch:
    ...
    def run(self, n_iter, save=True, plot=True, verbose=False):
        if plot:
            plt.ion()
            fig = plt.figure()
        ...
        rdg_dyn = linear_model.Ridge(alpha=self.dyn_reg, fit_intercept=False, tol=1e-4, solver="auto", max_iter=None)
        rdg_lgstc = linear_model.Ridge(alpha=self.lgstc_reg, fit_intercept=False, tol=1e-4, solver="auto", max_iter=None)
        rdg_ctl = linear_model.Ridge(alpha=self.ctl_reg, fit_intercept=False, tol=1e-4, solver="auto", max_iter=None)
