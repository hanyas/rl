    ...
    def run(self):
        ...
        for n in range(self.n_episodes):
            ...
            while not done:
                ...
                if not done:
                    self.qfunc[x, u] += self.alpha * (r + self.discount * np.max(self.qfunc[xn, :]) - self.qfunc[x, u])
                    self.td_error = np.append(self.td_error, r + self.discount * np.max(self.qfunc[xn, :]) - self.qfunc[x, u])
                if done:
