...
class MLPVFunc:
	...
	def evaluate_pol(self, paths, discount=.995, lam_trace=.95, nb_epochs=10, batch_size=64):
...
def rollout(env, policy, render=False):
	...
	while not done:
		...
		act = policy(obs)
		nobs, rwd, done, _ = env.step(act.reshape(1, 1))
...
# params
nb_iter = 35 # one iter -> at least min_trans_per_iter generated
