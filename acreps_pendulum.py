...
acreps = ACREPS(n_states=3, n_actions=1,
				n_samples=3000, n_iter=10, n_keep=0,
...
State = eval['xn'].reshape((-1, acreps.n_steps, acreps.n_states))
Angle = cart_polar(State)
Reward = eval['r'].reshape((-1, acreps.n_steps,))
Action = np.clip(eval['u'].reshape((-1, acreps.n_steps, acreps.n_actions)), -acreps.action_limit, acreps.action_limit)
...
for rollout in range(acreps.n_rollouts):
