...
from rl.hyreps.v1 import merge
...
class HyREPS_V:
    ...
    def __init__(self, env, n_regions,
        ...
        for n in range(self.n_regions):
            self.rslds.linear_ctls[n].K = copy.deepcopy(self.ctl.K[n])
            self.rslds.linear_ctls[n].cov = copy.deepcopy(self.ctl.cov[n])
    ...
    def ml_policy(self):
        ...
        for n in range(self.n_regions):
            self.ctl.K[n] = copy.deepcopy(bw.rslds.linear_ctls[n].K)
            self.ctl.cov[n] = copy.deepcopy(bw.rslds.linear_ctls[n].cov)
...
if __name__ == "__main__":
    ...
    ctl_prior = {'nu': 3, 'psi': 5.0}
    priors = [dyn_prior, ctl_prior]
    regs = np.array([np.finfo(float).tiny, 1e-12, 1e-12, 1e-6])

    hybrd_rslds = env.unwrapped.rslds

    hyreps = HyREPS_V(env, 2,
                      n_samples=2500, n_iter=10,
                      n_rollouts=100, n_steps=100, n_keep=0,
                      kl_bound=0.1, discount=0.97,
                      vreg=1e-12, preg=regs, cov0=5.0,
                      rslds=hybrd_rslds, priors=priors,
                      degree=2)

    for it in range(15):
        rwrd, kl_samples = hyreps.run()
