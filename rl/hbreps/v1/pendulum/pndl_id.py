...
if __name__ == "__main__":
    ...
    regs = np.array([np.finfo(np.float64).tiny, 1e-12, 1e-12, 1e-12])
    ...
    # test hybrid policy
    hyreps = HyREPS(env, n_regions, n_samples=5000,
    ...
    for n in range(n_regions):
        hyreps.ctl.K[n] = copy.deepcopy(bw.rslds.linear_ctls[n].K)
        hyreps.ctl.cov[n] = copy.deepcopy(bw.rslds.linear_ctls[n].cov)

    rollouts, eval = hyreps.evaluate(hyreps.n_rollouts, hyreps.n_steps, stoch=False)
