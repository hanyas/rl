...
if __name__ == "__main__":
    ...
    for i in range(npts):
        for j in range(npts):
            ...
            XYn[:, i, j], Un[i, j] = env.unwrapped.closed()
    ...
    # tikz_save("hybrid_dyn_closed.tex")

    fig, ax = plt.subplots()
    img = ax.imshow(Un, interpolation='bilinear',
                    cmap=cm.inferno, aspect='equal',
                    origin='lower', extent=[-5, 5, -5, 5],
                    vmax=abs(Un).max(), vmin=-abs(Un).max())

    cbar = fig.colorbar(img)
    plt.show()

    # from matplotlib2tikz import save as tikz_save
    # tikz_save("hybrid_ctl.tex")
